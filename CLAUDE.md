...
- `llm_news.py` — optional `LLMNewsAnalyzer` sends the same articles to the Claude API (one call per trading day, structured-outputs JSON schema, score −10…+10). Disabled by default; requires `ANTHROPIC_API_KEY` (a Console API key — a Claude Pro subscription cannot be used). Advisory unless `LLM_NEWS_BLOCK_ON_HIGH_RISK` is true. Do not add `temperature` or `thinking` params to its request — the model is user-configurable and those 400 or vary by model.
...
- Secrets never travel through Lumibot's `parameters` dict (it can be logged): `main.py` exports `ALPACA_API_KEY`/`ALPACA_API_SECRET` (read by `news_context.py` — alpaca-py has no env fallback of its own), `EMAIL_SMTP_PASSWORD` (read by `_send_daily_email`), and `ANTHROPIC_API_KEY` (read by `llm_news.py`). SMTP uses `ssl.create_default_context()` for STARTTLS — the stdlib default is unverified; don't regress it.
