...
2. **Market-level context** — `_get_news_context()`, `_symbol_news_scores()`, `_llm_assessment_for_iteration()` (always synchronous when enabled), `_run_pretrade_discovery_analysis()`, `_update_adaptive_learning()` / `_update_llm_adaptive_learning()`, `_market_veto_reason()` (world-event/LLM/learned-model block check — blocks **new** purchases/replacements only; completing an in-flight replacement or exit is never vetoed). `_refresh_symbol_reference()` and `_load_nightly_preeval_learnings()` (surfaces last night's overnight pass — see "Off-hours side channel" below — into the email) also run in this phase. `_check_discovery_red_flags` screens every negatively-covered symbol discovery just added this iteration (never held/static ones) and excludes severe company-specific risks. `_check_discovery_article_context` runs immediately after over the same candidates and excludes bearish full-article verdicts. Both complete before any order phase.
...
6. **Signals** — `_portfolio_signals(symbols)` (threaded per-symbol dip/edge computation via `_portfolio_signal`, measuring dips against the *previous* lookback bars excluding the event day's own high; also enforces a discovery liquidity floor — reject if price is below `PORTFOLIO_DISCOVERY_MIN_PRICE_DOLLARS` or recent average volume is below `PORTFOLIO_DISCOVERY_MIN_AVG_VOLUME` — applied to the whole evaluated universe, not just new discoveries), `_exclude_unpriceable_discovered_symbols()` (a discovery-only symbol with zero price history is permanently excluded via `AutonomousUniverse.exclude_unpriceable()`; a config-listed symbol never is), then per evaluated symbol: `_backfill_portfolio_memory()` + `_update_portfolio_memory()` (every evaluated symbol contributes a learning observation, not just qualifying ones — `signal_present` records whether today's dip actually cleared the live threshold) → `_posture_adjusted_edge()` (reshapes each candidate's raw historical edge through `PORTFOLIO_RISK_POSTURE`: `conservative`, default, penalizes return variance and a negative news-score day harder; `risky` barely discounts either) → `signal_snapshot.write_snapshot()` (every evaluated symbol's opinion — posture-adjusted edge if qualifying today, else the raw historical `expected_profit` — written to `.portfolio_signal_snapshot.json` for `scripts/web_dashboard.py`, purely observational, before eligibility trims the list) → eligibility filter (`qualifies`, min observations, `PORTFOLIO_MIN_EXPECTED_PROFIT_PERCENT`, out-of-sample floor — all computed from the raw `expected_profit`/`oos_expected_profit`, never shifted by posture or the learned edge) → sort.
...
flowchart TD
    ...
    subgraph SCREEN[Discovery screening & learning]
        direction TB
        RedFlags["_check_discovery_red_flags()<br/>every discovery-only symbol with negative score<br/>severe flags are excluded before orders"]
        ArticleCtx["_check_discovery_article_context()<br/>same discovery-only symbols, live —<br/>cache hit if the nightly pass already<br/>covered them; bearish verdicts are excluded"]
...
4. **Signals** — `_crypto_symbols()` (managed ∪ held ∪ one discovery batch), `_crypto_signals()`, unpriceable-discovery exclusion, then per evaluated symbol: `_backfill_crypto_memory()` + `_update_crypto_memory()` → eligibility filter → `posture_adjusted_edge()` (from `decision_math.py`, computed only for symbols passing every eligibility gate, unlike equity which computes it for every qualifying symbol before filtering) → sort → `signal_snapshot.write_snapshot()` (every evaluated symbol's opinion — posture-adjusted edge where computed, else the raw historical `expected_profit` — written to `.crypto_signal_snapshot.json` for `scripts/web_dashboard.py`, purely observational).
...
6. **Build** — empty-slot buys only (no replace/top-up), sized by `decision_math.optimal_position_count()` against `min(_account_half_value_dollars() - deployed, real-time cash)` — `_account_half_value_dollars()` always targets 50% of the shared account's live total value (crypto has no "is the other side enabled" check since equity always runs), recomputed every iteration rather than read from a fixed config value, which is the software-enforced cap that keeps crypto from spending into equity's own share. Equity's mirror-image method, `_crypto_reserve_dollars()` (`strategy.py`), instead returns zero whenever `crypto_enabled` is false, so a crypto-disabled deployment doesn't lose half its capital to an unused reservation (see `architecture.md`'s config section and the README's "Crypto trading mode").
...
- **Fail safely on data/API errors.** A broker read failure, missing quote, failed news/LLM call, or discovery outage is caught and logged rather than killing the process. Most paths fail open or skip the affected signal; the enabled LLM market guard is intentionally fail-closed when `LLM_NEWS_FAIL_CLOSED_ON_UNAVAILABLE=true`, blocking new exposure if the requested assessment is unavailable. Any uncaught iteration exception is logged and retried next cycle.
...
- **Decision-changing LLM work precedes new exposure.** The market assessment and discovery red-flag/article checks complete before opening orders. Only descriptive exit notes remain deferred to a background thread; they cannot affect a trade.
...
- **Per-iteration caches trade a few seconds of staleness for far fewer broker/DuckDB round-trips.** `_has_active_order()`'s underlying `get_orders()` result, and `_get_quote_price_and_bid_ask()`/`_get_crypto_quote_price_and_bid_ask()`'s per-symbol quote, are each cached for the duration of one `_run_portfolio_iteration`/`_run_crypto_iteration` call (cleared at the top of the function, and the orders cache also after every accepted order submission via `_submit_order_checked`) rather than refetched at every one of the several loop sites that call them. `AutonomousUniverse`, `SymbolReference`, and `TradeMemory` (via `_autonomous_universe()`/`_symbol_reference()`/`_decision_memory()` and their crypto mirrors) are similarly instance-cached per strategy object keyed by their constructor args, and each of those three classes now only creates its DuckDB schema once per instance (`_schema_initialized`) instead of on every method call — they still open a fresh `duckdb.connect()` per method call, so the existing concurrent-access handling between the main iteration thread and the broker-callback thread (`on_filled_order`) is unaffected.
- **Purely observational side channels never gate a decision.** `signal_snapshot.write_snapshot()` (per-symbol opinion, read by `scripts/web_dashboard.py`) and `trade_counter.record_trade()` (same-day trade count, also read by the dashboard) both run inline in the trading phases but only ever record what already happened; either one's write failure is swallowed inside itself, never propagated.
