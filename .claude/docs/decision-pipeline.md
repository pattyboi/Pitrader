...
6. **Build** — empty-slot buys only (no replace/top-up), sized by `decision_math.optimal_position_count()` against `min(_account_half_value_dollars() - deployed, real-time cash)` — `_account_half_value_dollars()` (identical method on both strategy classes) targets 50% of the shared account's live total value, recomputed every iteration rather than read from a fixed config value, which is the software-enforced cap that keeps crypto from spending into the equity strategy's own half (see `architecture.md`'s config section and the README's "Crypto trading mode").
