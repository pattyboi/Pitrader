...
return on (dip size, LLM opinion score). Implemented directly, no dependency, to
...
class TwoFeatureRidgeModel:
    ...
    def predict(self, dip: float, score: int | None) -> float:
        ...
        predicted += self.beta_opinion * ((score or 0) - self.mean_opinion)
...
def fit_two_feature_ridge_model(
    ...
    # Small ridge penalty makes a nearly constant LLM score harmless.
    a = centered_dip2 + 1.0
    ...
    beta_opinion = (a * centered_score_target - b * centered_dip_target) / determinant

    # Centered fitted values are beta_dip*x + beta_opinion*score. Their mean is
    # zero by construction, allowing correlation to use the same statistics.
    variance_fit = (
        beta_dip * beta_dip * centered_dip2
        + 2.0 * beta_dip * beta_opinion * centered_dip_score
        + beta_opinion * beta_opinion * centered_score2
    )
    covariance = beta_dip * centered_dip_target + beta_opinion * centered_score_target
    raw_correlation = (
...
def fit_two_feature_ridge(
    ...
    """Fit a ridge-stabilized 2-feature linear regression (dip, LLM score)
