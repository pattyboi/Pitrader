...
- `strategy_support.py` — `BrokerRuntimeSupport` (restart-safe state access, safe quantity parsing, one-read-per-iteration order cache, active-order checks, checked submission/trade counting), the `IterationContext` value object, and shared pooled-memory input/update helpers. It contains no equity/crypto selection policy.
